...
const sites = computed(
  () =>
    context.value?.sites.filter(
      (site) => !store.workspaceId || site.workspaceId === store.workspaceId,
    ) ?? [],
);
...
watch(
  () => store.workspaceId,
  () => {
    if (!sites.value.some((site) => site.id === store.siteId) && sites.value[0])
      store.setContext(store.workspaceId, sites.value[0].id);
...
onMounted(async () => {
  ...
  try {
    ...
    const workspace =
      context.value.workspaces.find((item) => item.id === store.workspaceId) ??
      context.value.workspaces[0];
    const site =
      context.value.sites.find((item) => item.id === store.siteId) ??
      context.value.sites.find((item) => item.workspaceId === workspace?.id);
    ...
    contextError.value =
      cause instanceof Error
        ? cause.message
        : "Platform context is unavailable";
  }
});
...
async function refreshJobs() {
  ...
  try {
    ...
    store.activeJobs = jobs.filter((job) =>
...
<template>
  <div
    class="app-shell"
    :class="{ 'app-shell--collapsed': store.sidebarCollapsed }"
  >
    <button
    ...
    <aside class="sidebar" :class="{ open: store.mobileNavigationOpen }">
      <div class="brand">
        <span class="brand-symbol"><PhTreeStructure :size="21" /></span>
      ...
      <div class="sidebar-foot">
        ...
        <div>
          <strong>{{ t("app.environment") }}</strong
          ><small>Protected governance boundary</small>
        </div>
      </div>
      <button
      ...
      >
        <component
          :is="store.sidebarCollapsed ? PhSidebarSimple : PhSidebar"
          :size="18"
        />
    ...
    <div class="shell-main">
      <header class="command-bar">
        <button
          ...
          @click="store.mobileNavigationOpen = true"
        >
          <PhList :size="22" />
        </button>
        <div class="context-selectors">
          <label>
            <span>Workspace</span>
            <select v-model="store.workspaceId" aria-label="Workspace">
              <option
          ...
          <label>
            <span>Site</span>
            <select v-model="store.siteId" aria-label="Site">
        ...
        <button class="command-search" type="button" @click="openPalette">
          <PhMagnifyingGlass :size="18" /><span>{{ t("app.search") }}</span
          ><kbd>⌘ K</kbd>
        </button>
        <RouterLink class="job-indicator" to="/operations"
          ><span :class="{ pulse: store.activeJobs }" />{{ store.activeJobs }}
          {{ t("app.jobs") }}</RouterLink
        >
        <button
    ...
    <Teleport to="body">
      ...
      >
        <section
          ...
          role="dialog"
          aria-modal="true"
          :aria-label="t('app.commandTitle')"
        >
          <header>
            <span class="command-mark"><PhCommand :size="20" /></span>
            <div>
              <strong>{{ t("app.commandTitle") }}</strong
              ><small>{{ t("app.commandHint") }}</small>
            </div>
            <kbd>ESC</kbd>
          </header>
          <label class="command-input"
            ><PhMagnifyingGlass :size="20" /><input
              ref="commandInput"
              v-model="commandQuery"
              :placeholder="t('app.search')"
              @keydown.enter="searchDecisions"
          /></label>
          <div class="command-results">
            ...
            >
              <span
                ><component :is="item.icon" :size="19" />{{ item.label }}</span
              ><PhArrowRight :size="17" />
            </button>
            <button
              v-if="commandQuery.trim()"
              class="command-search-action"
              type="button"
              @click="searchDecisions"
            >
              <span
                ><PhMagnifyingGlass :size="19" />Search decisions for
                “{{ commandQuery.trim() }}”</span
              ><kbd>↵</kbd>
            </button>
          </div>
        </section>
