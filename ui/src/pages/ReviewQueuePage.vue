...
import { PhClipboardText, PhFunnel, PhCircleNotch } from "@phosphor-icons/vue";
...
async function load() {
  ...
  error.value = "";
  try {
    items.value = await api.reviewItems(store.siteId);
  } catch (cause) {
    error.value =
      cause instanceof Error ? cause.message : "Review queue unavailable";
  } finally {
    loading.value = false;
  }
}
async function dispose(status: "ACCEPTED" | "DEFERRED" | "REJECTED") {
  ...
  error.value = "";
  try {
    const dispositions = [...selected.value].map((itemId) => ({
      itemId,
      status,
      reason,
    }));
    ...
    error.value = cause instanceof Error ? cause.message : "Disposition failed";
  } finally {
    saving.value = false;
  }
}
...
<template>
  <section>
    <header class="page-header">
      <div>
        <p class="page-kicker">Human disposition</p>
        <h1>Review queue</h1>
        <p>
          Resolve unmapped fragments and extraction diagnostics before
          promotion.
        </p>
      </div>
      <button class="secondary-button"><PhFunnel :size="15" />Filters</button>
    </header>
    ...
    <section class="surface table-surface">
      <div class="table-toolbar">
        ...
        <div>
          <PhCircleNotch v-if="saving" class="spin" :size="16" /><button
            class="secondary-button"
            :disabled="!selected.size || saving"
            @click="dispose('ACCEPTED')"
          >
            Accept</button
          ><button
            class="secondary-button"
            :disabled="!selected.size || saving"
            @click="dispose('DEFERRED')"
          >
            Defer</button
          ><button
            class="danger-button"
            :disabled="!selected.size || saving"
            @click="dispose('REJECTED')"
          >
            Reject
          </button>
        </div>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="item in items" :key="String(item.id)">
              ...
              <td>{{ item.adapter }}</td>
              <td>
                <code>{{ String(item.rawFragment).slice(0, 100) }}</code>
              </td>
              <td>
                <span
                  class="status-badge"
